...
class ExperimentRunner:
    ...
    def run(self, train_dataloader, val_dataloader, test_dataloader):
        ...
        # 检查是否是不微调实验
        if self.config.experiment_type == "no_finetune":
            return self._run_no_finetune_experiment(val_dataloader, test_dataloader)
        else:
            # 其他实验类型使用原来的训练流程
            return self._run_normal_experiment(train_dataloader, val_dataloader, test_dataloader)
    
    def _run_no_finetune_experiment(self, val_dataloader, test_dataloader):
        """
        运行不微调实验（跳过训练，直接评估）
        ...
        experiment_result = self._save_experiment_results(
            model, training_stats, test_results, best_epoch=1, best_val_loss=val_results['loss']
        )
        ...
        logger.info(f"✅ 实验 {self.config.name} 完成!")
        
        return experiment_result
    
    def _run_normal_experiment(self, train_dataloader, val_dataloader, test_dataloader):
        """
        运行正常训练实验（原训练结构保持不变）
        ...
        Returns:
            实验结果字典
        """
        logger.info(f"实验类型: {self.config.experiment_type}")
